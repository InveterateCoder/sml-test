...
async function renderController(req, res) {
  try {
    const index = await (0,fs_promises__WEBPACK_IMPORTED_MODULE_1__.readFile)(path__WEBPACK_IMPORTED_MODULE_0___default().resolve(__dirname, '..', '..', 'client', 'build', 'index.html'), {
      encoding: 'utf8'
    });
    res.writeHead(200, {
...
async function startServer() {
  ...
  try {
    app.use(express__WEBPACK_IMPORTED_MODULE_3___default().static(path__WEBPACK_IMPORTED_MODULE_0___default().resolve(__dirname, '..', '..', 'client', 'build'))).use(express__WEBPACK_IMPORTED_MODULE_3___default().json()).use(express__WEBPACK_IMPORTED_MODULE_3___default().urlencoded({
      extended: true
